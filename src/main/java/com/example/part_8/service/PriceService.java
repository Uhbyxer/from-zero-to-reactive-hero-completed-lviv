...
//        return Flux.never();
        ...
        return requestedInterval
                .mergeWith(Flux.just(DEFAULT_AVG_PRICE_INTERVAL))
                .switchMap(timeWindow -> priceData.window(Duration.ofSeconds(timeWindow)))
                .flatMap(buffer -> buffer
                        .groupBy(MessageDTO::getCurrency)
                        .flatMap(oneCurrencyFlux -> oneCurrencyFlux
                                .map(MessageDTO::getData)
                                .reduce(Sum.empty(), Sum::add)
                                .filter(sum -> sum.getCounter() > 0)
                                .map(sum -> MessageDTO.avg(sum.avg(), oneCurrencyFlux.key(), "External"))
                        )
                );
